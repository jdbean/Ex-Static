module ExStatic
  module PathHelper
    def sanitized_path(path)
      ...
      Jekyll.sanitized_path ExStatic.site.source, path
    end
    ...
    def validate_path(path)
      ...
      if !path.start_with?("#{ExStatic.site.source}/_posts")
        Jekll.logger.debug "INVALID:", path
        render_404
      else
      end
    end
    ...
    def path_without_site_source(path)
      path.to_s.gsub(%r!\A#{Regexp.escape(ExStatic.site.source)}!, "")
    end
  end
end
